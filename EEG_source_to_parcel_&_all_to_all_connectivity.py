...
CrossCorr_thresholded_matrix = graph_to_matrix(G_CrossCorr_thresholded, CrossCorr_matrix.shape[0])
...
plot_matrix(CrossCorr_thresholded_matrix, f'Thresholded Cross-Correlation Matrix for Window {chosen_window}', ordered_regions)

# Dynamic representation (animation) across all windows for heatmat
from matplotlib.animation import FuncAnimation

fig, ax = plt.subplots(figsize=(10, 10))
sns.heatmap(graph_to_matrix(G_CrossCorr_thresholded, CrossCorr_matrix.shape[0]), cmap='viridis', xticklabels=ordered_regions, yticklabels=ordered_regions, ax=ax)
title = ax.set_title(f'Thresholded Cross-Correlation Matrix for Window 0')

def update(window_number):
    ax.clear()
    current_matrix = graph_to_matrix(nx.convert_matrix.from_numpy_array(windowed_cross_correlation_matrices[window_number]), windowed_cross_correlation_matrices[window_number].shape[0])
    sns.heatmap(current_matrix, cmap='viridis', xticklabels=ordered_regions, yticklabels=ordered_regions, ax=ax)
    title.set_text(f'Thresholded Cross-Correlation Matrix for Window {window_number}')
    return ax,
