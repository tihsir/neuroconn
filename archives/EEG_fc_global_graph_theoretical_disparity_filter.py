...
def visualize_graph_colored_nodes(G, title="Graph Visualization with Colored Nodes"):
    ...
    if G.is_directed():
        nx.draw_networkx_edges(
            G, layout, width=1.0, alpha=0.5, ax=ax, arrowstyle='-|>', arrowsize=20)
    else:
    ...
    sm = plt.cm.ScalarMappable(cmap=plt.cm.viridis, norm=mcolors.Normalize(
        vmin=min(degrees), vmax=max(degrees)))
    sm.set_array([])
    cbar = plt.colorbar(
        sm, ax=ax, label='Node Degree (In-degree for directed graphs)')
...
visualize_graph_colored_nodes(
    G_dPLI_thresholded, title="dPLI Directed Graph Visualization with Colored Nodes")

# Visualize the PLI undirected graph
visualize_graph_colored_nodes(
    G_PLI_thresholded, title="PLI Undirected Graph Visualization with Colored Nodes")
